'''
Step V
Step 5 is almost identical to step 2. 
...
with open(f'{path_pages}/pages_dict.json') as fp:
...
def parse_offer_pages(folder_path: str) -> pd.DataFrame:
	...
	status_df = pd.DataFrame(columns=['execution_date', 'offer_id', 'offer_url', 'status'])
	...
	offer_pages = [ fname for fname in all_files if fname.endswith('.html')]
	
	offer_pages.sort()
	
	# offer_pages = ['page_1.html'] # here was a mistake
...
def main():
	...
	print(f'**********************************')
	print(f'STAGE 9: Parsing saved offer pages')
	print(f'**********************************')
	df_base = parse_offer_pages(os.path.join(path_pages))
	...
	print(f'**********************************')
	print(f'STAGE 10: Saving results to Azure')
	print(f'**********************************')
